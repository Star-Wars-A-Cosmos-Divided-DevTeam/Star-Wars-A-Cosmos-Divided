...
PIX_OUTPUT pix(in VERT_OUTPUT_BEAM input) : SV_TARGET
{
	...
	float4 colorA = float4(0.213, 0.5, 0.219, 1); //desired color * 0.5
	float4 green = float4(0, 1, 0, 1);
	float4 layer1 = colorA * innerCombined * innerSharp; //tinting the processed noise and masking it with innerSharp
	float4 col = lerp(green, layer1, innerGrad); //blending in green using a cylinder gradient
	col = lerp(green * noiseBase, col, innerBeam); //breaking up the outline
