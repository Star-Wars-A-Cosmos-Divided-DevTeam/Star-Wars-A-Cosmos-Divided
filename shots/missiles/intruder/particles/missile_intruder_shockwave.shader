...
PIX_OUTPUT pix(in VERT_OUTPUT2 input) : SV_TARGET
{
	...
	float4 col;
	col.rgb = bbAvg * float3(0, 1, 1) * 1.5; // Emphasize cyan color
	ret.rgb = lerp(ret.rgb, col.rgb + ret.rgb, saturate(src.a * strength * 20));

	// Add some additional glow to the cyan shockwave
	float4 glow = float4(0, 1, 1, 1) * (1.0 - dist) * 0.5 * src.a;
	ret.rgb += glow.rgb * glow.a;
