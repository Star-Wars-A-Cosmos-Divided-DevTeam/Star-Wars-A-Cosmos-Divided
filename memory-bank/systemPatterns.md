...
- `<./Data/...>` points at Cosmoteer’s built-in rules; treat these as read-only parents when inheriting.
...
- See `memory-bank\docs\Cosmoteer – Rules Language & Style.md` for more information.
...
- `strings/*.rules` supply localized text (en, de, es, fr, it, ja, pt-br, ru, tr, zh-cn) for every referenced key. See `memory-bank\docs\Cosmoteer – Strings Guide.md` For more details. 
